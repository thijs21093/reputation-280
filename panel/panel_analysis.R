...
## TO DO: Hearing to NA for month for which there is no data!
tdata <- data %>%
  dplyr::mutate(media_threat = case_when(
    media_threat == "no" ~ 0,
    media_threat == "yes" ~ 1),
  audience_threat = case_when(
    audience_threat == "no" ~ 0,
    audience_threat == "yes" ~ 1),
  information.cgm = center(information, "CGM"),
  valence.3.cgm = center(valence.3, "CGM"),
  valence.3.cwc = center(valence.3, "CWC", cluster = data$agencyname),
  valence.12.cgm = center(valence.12, "CGM"),
  audience_count.lcgm = center(log2(audience_count + 1), "CGM"),
  audience_count.l = log2(audience_count + 1),
  audience_count.lcwc = center(log2(audience_count + 1), "CWC", cluster = data$agencyname),
  media_count.lcgm = center(log2(media_count + 1), "CGM"), 
  media_count.l = log2(media_count + 1), 
  media_count.lcwc = center(log2(media_count + 1), "CWC", cluster = data$agencyname), 
  media_count.cgm = center(media_count, "CGM"),
  media_count.3.cgm = center(media_count.3, "CGM"),
  media_count.12.cgm = center(media_count.12, "CGM"),
  audience_threat.cgm = center(audience_threat, "CGM"),
  media_threat.cgm = center(media_threat, "CGM"),
  month.num = as.numeric(as.factor(month))) %>%
  group_by(agencyname) %>%
 mutate(random.num = sample(1:1500, n(), replace = FALSE))

pdata <- pdata.frame(tdata, index = c("agencyname","month"),
            drop.index = TRUE, row.names = TRUE)
  
pdim(pdata) # Dimensions
head(index(pdata)) # A look at the data
